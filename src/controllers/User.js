    ...
    try {
        ...
        if (userdata) {
            if (await bcrypt.compare(password, userdata.password)) {
                const token = jwt.sign({ uid: userdata._id }, process.env.JWT_SECRET_TOKEN);
...
const userInviteDashboard = async (req, res) => {
    console.log("hello");
    ...
    try {
      ...
      const mailOptions = {
        ...
        text: 'You have been invited to a workspace. Please click the link to join: [Your Workspace Link]',
        html: '<p>You have been invited to a workspace. Please click the link to join: <a href="[Your Workspace Link]">Join Workspace</a></p>',
      };
      // Send the email
...
module.exports = { loginUser, registerUser, updateUser, userDashboard, userInviteDashboard };